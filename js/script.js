...
const App = {
  ...
  renderExamList() {
    ...
    const order = ['Competitive Exams', 'Board Exams'];
    const groups = {};
    ...
    Object.keys(groups).forEach(cat => {
      groups[cat].sort((a, b) => {
        const rank = (e) => {
          ...
          if (n.includes('NEET') && !n.includes('PG')) return 0;
          if (n.includes('JEE MAIN') && n.includes('SESSION 1')) return 1;
          if (n.includes('JEE MAIN') && n.includes('SESSION 2')) return 2;
          if (n.includes('JEE ADVANCED')) return 3;
          if (n.includes('JEE')) return 4;
          if (e.important) return 5;
          return 10;
        };
    ...
    container.innerHTML = cats.map(cat => {
      const cards = groups[cat].map(exam => {
      ...
      return `<p class="section-label">${this.escape(cat)}</p>${cards}`;
    }).join('');
